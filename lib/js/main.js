...
let userlistenedsongsurl = []
if (dbGetItem("userlistenedsongsurl") === null) {
    userlistenedsongsurl = []
} else {
    userlistenedsongsurl = dbGetItem("userlistenedsongsurl").split("\n")
}
let usermouseX = 0;
let usermouseY = 0;
let userlastvisitedpage = ""
if (dbGetItem("userlastvisitedpage") === null) {
    userlastvisitedpage = "https://soundcloud.com/discover"
    dbSetItem("userlastvisitedpage", userlastvisitedpage)
} else {
    userlastvisitedpage = dbGetItem("userlastvisitedpage")
}

let webview = document.querySelector("#webview")
if (settings.startuplastpage === "true") {
    webview.setAttribute("src", userlastvisitedpage)
} else {
...
webview.addEventListener('dom-ready', () => {
    ...
    webview.setZoomFactor(parseFloat(settings.zoomfactor))

    reloadTimeoutState = true
})

webview.addEventListener('did-redirect-navigation', () => {
    console.log("Navigated")
    userlastvisitedpage = webview.getURL()
    dbSetItem("userlastvisitedpage", userlastvisitedpage)
})
...
webview.addEventListener('console-message', (e) => {
    if (e.level == 1) {
        ...
        if (signalParts[0] == "BSCReceive") {
            ...
            } else if (signalParts[1] == "CurSongUrl") {
                if (!userlistenedsongsurl.includes(signalParts[2])) {
                    userlistenedsongsurl.push(signalParts[2])
                    dbSetItem("userlistenedsongsurl", userlistenedsongsurl.join("\n"))
                }
...
if (adSystemState == "default") {
    adSystemDivs[0].classList.add("active")
} else if (adSystemState == "silentAds") {
...
setInterval(() => {
    ...
    if (userlistening) {
        showcasenavplay.classList.remove("tglplayshowcasebtnpaused")
    } else { showcasenavplay.classList.add("tglplayshowcasebtnpaused") }

    if (userlisteningliked == "true") {
        showcasenavlike.classList.add("tgllikeshowcasebtnliked")
...
function lyricShowCaseUpdate() {
    ...
    userlastlyricshowcasesong = userlisteningartist + " - " + userlisteningsong
    ...
    getLyrics(geniusoptions).then((lyrics) => {
        if (lyrics === null) {
            userlisteninglyrics = "Lyrics Not Found"
        } else {
            userlisteninglyrics = lyrics
        }
        let lyricalignmentrtl = false
        if (userlisteninglyrics.includes("ا")) {
            lyricalignmentrtl = true
        }
        if (userlisteninglyrics === "Lyrics Not Found") {
...
setInterval(() => {
    if (userlastlyricshowcasesong !== userlisteningartist + " - " + userlisteningsong) {
        lyricShowCaseUpdate()
...
// Handle Deep Links
ipcRenderer.on("appDeepUrl", function (evt, message) {
    var theurl = message.replace("bsc", "https")
    console.log(theurl)
    if (theurl.includes("soundcloud.com")) {
...
let scrollreqloop;
function scrollReq() {
    execJS(`window.scrollBy(0, 100)`)
}
function scrollBtnClick() {
    if (!scrollerBtn.classList.contains("scrollerbtnactive")) {
        scrollerBtn.classList.add("scrollerbtnactive")
        scrollreqloop = setInterval(() => {
            scrollReq()
        }, 10)
    } else {
...
// Check Trayicon
if (!fs.existsSync("C://BetterSoundCloud/assets/bw-icon.ico")) {
    ipcRenderer.send("download", {
        url: "https://easyfiles.cc/2024/12/db8a2c79-7934-4b36-bf8b-91fd0545b456/bw-icon.ico",
        properties: { directory: "C://BetterSoundCloud/assets" }
    });
}